...
> **Resumo normativo das regras**: [`conventions.md`](conventions.md) — fonte única do "o quê / deve" (naming, camadas, DI, erros, Pydantic, UUID, ban de `Any`, testes, logger, OpenAPI, comandos, estrutura). **Este documento é o detalhamento** (exemplos, templates, rationale) dessas regras; em caso de divergência de enunciado, `conventions.md` prevalece.
...
Cada feature vive em `src/modules/<feature>/` com layout em camadas + pasta versionada de controllers. O diretório do módulo é **plural** (`<feature>`, ex.: `users`); os arquivos são prefixados pelo **singular** da entidade (`<entity>`, ex.: `user`):
...
├── <entity>_router.py                # agrega controllers/v1/ (e futuras v2/, v3/)
├── <entity>_service.py               # regra de negócio — lança DomainError
├── <entity>_repository.py            # acesso a Prisma — Protocol + implementação
├── <entity>_schema.py                # DTOs Pydantic v2 — *Request / *Response
└── <entity>_dependencies.py          # FastAPI Depends() — wiring repository → service
```

Exemplo concreto (`users`): `user_service.py`, `user_repository.py`, `user_schema.py`, `user_dependencies.py`, `user_router.py`, `controllers/v1/user_controller.py`.

### Versionamento de URL

- O prefixo `/api/v1` é aplicado **em [src/main.py](../../src/main.py)** no `app.include_router(<feature>_router, prefix="/api/v1")` — não dentro do controller. Isso concentra a versão em um lugar só; um futuro `controllers/v2/` reaproveita o mesmo `<entity>_router.py` do módulo e ganha seu próprio `include_router(..., prefix="/api/v2")` em `main.py`.
...
- **Arquivos em `snake_case` prefixados pela entidade.** Cada arquivo de camada nomeia a entidade (singular) + a camada: `<entity>_<layer>.py`. O separador é **`_` (underscore), nunca `.`** — em Python o ponto é separador de pacote e `user.service.py` **não é importável**.
  - ✅ `src/modules/users/user_service.py`
  - ✅ `src/modules/orders/controllers/v1/order_controller.py`
  - ❌ `src/modules/orders/controllers/v1/order.controller.py` (ponto quebra o import)
  - ❌ `src/modules/orders/orderController.py` (camelCase)
  - ❌ `src/modules/orders/service.py` (sem prefixo da entidade)
- O arquivo dentro de `controllers/v1/` é `<entity>_controller.py`; permite no futuro adicionar mais recursos na mesma `v1` (ex.: `session_controller.py`) sem ambiguidade.
- **Diretório do módulo em snake_case**, plural para recursos (`orders`, `products`), singular para conceitos (`auth`, `billing`). O **prefixo do arquivo** usa o singular da entidade (`order_service.py` em `modules/orders/`).
- **Nomes de classe** permanecem `PascalCase` prefixados pela entidade: `UserService`, `UserRepository`, `UserRepositoryProtocol`, `CreateUserRequest`. Apenas o nome do **arquivo** carrega o prefixo `snake_case`.

> **Histórico**: `src/modules/users/` usou primeiro o estilo NestJS dotted (`users.module.py`), depois o padrão flat sem prefixo (`service.py`, migração de 2026-05-20). Em **2026-05-27** adotou-se o naming `snake_case` prefixado pela entidade (`user_service.py`) — equivalente Pythonico (importável) do estilo `*.service.*` do NestJS. Decisão registrada no [ADR 0004](../adr/0004-naming-com-prefixo-de-entidade.md). Não reintroduzir naming flat sem prefixo nem dotted.
...
### Controller (`controllers/v1/<entity>_controller.py`)
...
- Métodos retornam `dict[str, object]` ou `dict[str, object] | None` — resultado bruto do Prisma serializado via `.model_dump()` (não devolver `prisma.models.*`, senão o service precisaria importar Prisma). Conversão para Pydantic é responsabilidade do service.
- **Use `dict[str, object]`, nunca `dict` sem parâmetros** — o `mypy --strict` do projeto liga `disallow_any_generics` e rejeita generics não parametrizados.
- I/O puro. Sem regra de negócio (cálculo de total, checagem de status, etc.).

### Schema (`<entity>_schema.py`)
...
### Dependencies (`<entity>_dependencies.py`)
...
- `<entity>_router.py` agrega os controllers versionados do módulo. Para uma única `v1`:
...
from src.modules.users.user_schema import CreateUserRequest, UserResponse
...
> O arquivo acima vive em `src/modules/users/controllers/v1/user_controller.py`. O `prefix="/users"` é apenas do recurso — o `/api/v1` é aplicado em [src/main.py](../../src/main.py) na hora do `include_router`. URL final: `POST /api/v1/users`.
...
**Apenas o `<entity>_repository.py` importa Prisma.** Service e controller dependem do repository via `Protocol`. Isso:
...
```python
# src/modules/<feature>/<entity>_repository.py
...
class <Feature>Repository:
    ...
    async def create(self, data: Create<Feature>Request) -> dict[str, object]:
        created = await self._db.<feature>.create(data={"name": data.name})
        return created.model_dump()

    async def find_by_id(self, <feature>_id: UUID) -> dict[str, object] | None:
        row = await self._db.<feature>.find_unique(where={"id": str(<feature>_id)})
...
> **Tipar args do Prisma com `types.*` — nunca `cast(Any, ...)`.** O pacote `prisma` tem `py.typed`; em mypy `strict`, dicts literais para `data`/`where`/`include`/`order` podem não casar com os `TypedDict` gerados. A solução **não** é `cast(Any, ...)` (isso desliga a checagem) e sim **anotar variáveis** com os types gerados (`from prisma import types`):
>
> ```python
> async def create(self, *, email: str, role: UserRole) -> dict[str, object]:
...
> ```
>
> Filtro de relação no `where` usa a forma canônica `{"role": {"is": {...}}}` (tipo `RoleRelationFilter`), não o atalho `{"role": {...}}`. `Any` é banido no projeto (ruff `TID251`); ver [AGENTS.md](../../AGENTS.md).

### 5.5. Tipos de retorno

- **Repository retorna `dict[str, object]` ou `dict[str, object] | None`** — o resultado bruto do Prisma via `.model_dump()`. **Nunca Pydantic nem `prisma.models.*`.** (`dict` sem parâmetros falha no `mypy --strict`.)
- O **service** converte para `*Response` via `<Response>.model_validate(raw)`.
- **Quando o nome dos campos diverge** (colunas com `@map` viram atributos camelCase no Prisma — `isActive`, `createdAt` — enquanto o `*Response` é snake_case), **ou há relação aninhada** (ex.: `role` vira `{"name": ...}`), `model_validate(raw)` direto **não** mapeia. Nesse caso o service remapeia as chaves para snake_case e achata a relação antes do `model_validate` — sem reintroduzir import de Prisma:

  ```python
  def _to_response(self, raw: dict[str, object]) -> UserResponse:
      role = raw.get("role")
      if not isinstance(role, dict):
          # invariante de servidor: logar o detalhe, responder genérico (não vazar ao cliente)
...
```python
async def find_with_items(self, order_id: UUID) -> dict[str, object] | None:
    row = await self._db.order.find_unique(
...
async def create_order_with_items(
    self,
    order_data: types.OrderCreateInput,
    items_data: list[types.OrderItemCreateWithoutRelationsInput],
) -> dict[str, object]:
    async with self._db.tx() as tx:
        ...
        created = await tx.order.find_unique(
            where={"id": order.id},
            include={"items": True},
        )
        assert created is not None  # acabou de ser criado na mesma tx
        return created.model_dump()
```
...
class UserRepository:
    ...
    async def create(self, data: CreateUserRequest) -> dict[str, object]:
        try:
            created = await self._db.user.create(
...
- **Soft delete** quando aplicável: `deletedAt DateTime? @map("deleted_at")` + queries com `where: {"deletedAt": None}`. **Toda tabela com soft delete é obrigada a indexar a coluna** (`@@index([deletedAt])`): `deletedAt` entra em *todo* `where` da tabela, então é filtro quente por definição. Ver `User` em [schema.prisma](../../src/infra/prisma/schema.prisma).
...
Composição manual com `Depends` do FastAPI — **sem framework de DI externo**. O wiring vive em `<entity>_dependencies.py` por módulo. Use `Annotated[T, Depends(...)]` para todos os providers e handlers:

```python
# src/modules/<feature>/<entity>_dependencies.py
from typing import Annotated
...
from src.modules.<feature>.<entity>_repository import (
    <Feature>Repository,
...
1. **Chamada direta via Protocol importado.** `OrderService` recebe `UserRepositoryProtocol` (importado de `src/modules/users/user_repository.py`) via `__init__`. O wiring é feito no `<entity>_dependencies.py` do consumidor. Síncrono, fortemente tipado, simples — apropriado para a maioria dos casos atuais.
...
Objetos de mock e DTOs reaproveitados entre testes são criados exclusivamente por **factory functions** em `test/factories/<entity>_factory.py`, expostas pelo barrel `test/factories/__init__.py`. Testes nunca redefinem helpers locais (`_raw_*`, `_make_*`, `_seed_*`, `_user_response`, etc.) quando já existe factory para a entidade.

- **Localização**: `test/factories/<entity>_factory.py` (espelha `src/modules/<feature>/`). Barrel `test/factories/__init__.py` re-exporta as factories públicas para imports curtos: `from test.factories import make_user_row`.
...
> Adicionou uma feature nova? Criar `test/factories/<entity>_factory.py` com as três variantes e registrar o barrel no mesmo PR — a skill `create-module` já gera o scaffold.
...
- Algum módulo passa a ter > 10 endpoints e a regra de negócio fica densa demais para um único `<entity>_service.py`.
